...
def pollpage(): #page for polling
     def proceed():
        ...
        pd.commit()
        CTkMessagebox.showinfo('Success!','You have voted')
     choose=StringVar()
     names=[]
...
def polls(): #mypolls
    def proceed():
        ...
        plname=psel.get()
        if plname=='-select-':
            return CTkMessagebox.showerror('Error','select poll')
        else:
            mpolls.destroy()
            pollpage()
    ...
    Label(mpolls,text='Select Poll').grid(row=1,column=3)
    select= ctk.CTkComboBox(mpolls,values=pollnames,state='readonly')
...
def create():
    def proceed():
        ...
        if pname=='':
            return CTkMessagebox.showerror('Error','Enter poll name')
        elif can=='':
            return CTkMessagebox.showerror('Error','Enter candidates')
        else:
            ...
            for i in range(len(candidates)):
                ...
                pd.commit()
            pd.close()
            CTkMessagebox(title='Polling Sytem', message= 'Success!\nPoll Created')
            cr.destroy()
...
def selpl(): #pollresults
    ...
    sel=ctk.Combobox(pl,values=pollnames,state='readonly',textvariable=sele)
    sel.grid(row=2,column=1)
    sel.current(0)
    Button(pl,text='Get Results',command=results).grid(row=2,column=2)
def about():
    CTkMessagebox.showinfo('About','The Exceptional Coders Group')


home=Tk()
home.geometry('400x400')
home.title('Exceptional coders')
home['bg'] = '#49A'
